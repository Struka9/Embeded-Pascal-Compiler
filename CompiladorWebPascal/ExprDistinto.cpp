...
ExprDistinto::ExprDistinto(Expresion* izq,Expresion* der)
{
	SetOpOne(izq);
	SetOpTwo(der);
}
...
ExprDistinto::~ExprDistinto(void)
{
	delete op1;
	delete op2;
}

Tipo* ExprDistinto::validarSemantico()
{
...
Valor* ExprDistinto::interpretar()
{
	...
	else if ( v_expr1->GetTipoValor()==cadena && v_expr2->GetTipoValor()==cadena )
	{
		...
		return new ValorBooleano( v_str_expr1->GetValor().compare( v_str_expr2->GetValor() ) != 0 );
	}
	else if ( v_expr1->GetTipoValor()==cadena && v_expr2->GetTipoValor()==caracter )
	{
		...
		return new ValorBooleano( v_str_expr1->GetValor().compare( v_char_expr2->GetVal() ) != 0 );
	}
	else if ( v_expr1->GetTipoValor()==caracter && v_expr2->GetTipoValor()==cadena )
	{
		...
		return new ValorBooleano( v_char_expr1->GetVal().compare( v_str_expr2->GetValor() ) != 0 );
	}
	else if ( v_expr1->GetTipoValor()==caracter && v_expr2->GetTipoValor()==caracter )
	{
		...
		return new ValorBooleano( v_str_expr1->GetVal().compare( v_str_expr2->GetVal() ) != 0 );
	}
	else
	{
		ValorBooleano* v_bool_expr1 = (ValorBooleano*)v_expr1;
		ValorBooleano* v_bool_expr2 = (ValorBooleano*)v_expr2;

		return new ValorBooleano( v_bool_expr1->GetVal() != v_bool_expr2->GetVal() );
	}
